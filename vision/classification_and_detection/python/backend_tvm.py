...
class BackendTVM(backend.Backend):
    ...
    def load_impl(self, model_path, inputs, outputs, max_batchsize):
        ...
        if os.path.isfile(compiled_model):
            ...
            tvm_history_json_file = os.environ.get(
                'MLPERF_TVM_APPLY_HISTORY', '').strip()
            if tvm_history_json_file != '':
                if not os.path.isfile(tvm_history_json_file):
                    print('')
                    raise Exception("Error: TVM history file {} not found!".format(
                        tvm_history_json_file))
...
'
        if self.arena_num > 1:
            self.pool = pool.ProcessPoolExecutor(self.arena_num,
    ...
    def predict(self, feed):
        """Run the prediction."""
        if self.arena_num > 1:
            return self.pool.submit(self._worker_handler, feed).result()
        else:
